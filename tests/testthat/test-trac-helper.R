...
test_that("Construct A for non compositional data works", {
  # construct A with 2 compositional hierarchical features
  ...
  # create a non compositional and non hierarchical feature named p1
  ...
  expect_equal(unname(as.matrix(A_add_X(X, A, p, p_x))),
               test_A)
  expect_equal(rownames(A_add_X(X, A, p, p_x)),
               test_A_names)
  })
